...
{
	struct bt_conn *conn;
	struct bt_conn_le_cs_capabilities_v2 remote_cs_capabilities;
	...
	if (evt->status == BT_HCI_ERR_SUCCESS) {
		...
		remote_cs_capabilities.t_ip2_ipt_times_supported = evt->t_ip2_ipt_times_supported;
		remote_cs_capabilities.t_sw_ipt_time_supported = evt->t_sw_ipt_time_supported;

		bt_conn_notify_remote_cs_capabilities_v2(conn, BT_HCI_ERR_SUCCESS,
						      &remote_cs_capabilities);
	} else {
		bt_conn_notify_remote_cs_capabilities_v2(conn, evt->status, NULL);
	}
...
int bt_le_cs_read_local_supported_capabilities_v2(struct bt_conn_le_cs_capabilities_v2 *ret)
{
	struct bt_hci_rp_le_read_local_supported_capabilities_v2 *rp;
	struct net_buf *rsp;

	int err =
		bt_hci_cmd_send_sync(BT_HCI_OP_LE_CS_READ_LOCAL_SUPPORTED_CAPABILITIES_V2, NULL, &rsp);
...
int bt_le_cs_write_cached_remote_supported_capabilities_v2(struct bt_conn *conn,
        const struct bt_conn_le_cs_capabilities_v2 *params)
{

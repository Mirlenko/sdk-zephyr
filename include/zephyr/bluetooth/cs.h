...
/** @brief Enable Inline PCT Transfer (IPT)
 *
 * This command is used to enable Inline PCT Transfer (IPT) for the connection
 * identified by the conn parameter.
 *
 * @note To use this API @kconfig{CONFIG_BT_CHANNEL_SOUNDING} must be set.
 *
 * @param conn      Connection Object.
 ...
 * @return Zero on success or (negative) error code on failure.
 */
 int bt_le_cs_ipt_enable(struct bt_conn *conn, uint8_t enable);
 ...
 */
 int bt_le_cs_write_cached_remote_supported_capabilities_v2(
	struct bt_conn *conn, const struct bt_conn_le_cs_capabilities_v2 *params);
